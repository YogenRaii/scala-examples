...
object Anagrams {
  ...
  lazy val allOccurrences = dictionary map wordOccurrences
    ...
    if (sentence.isEmpty) List(Nil)
      ...
      def process(list: List[Occurrences], sentence: Sentence): List[Sentence] = {
        if (list isEmpty) {
          ...
          val head :: tail = list
          val notYetUsed = dictionaryByOccurrences(head)
  ...
  def goodFor(got: Occurrences): (Occurrences => Boolean) = {
    val occurenceMap = (got toMap) withDefaultValue 0

    def good(dictionaryRecord: Occurrences): Boolean = {
      dictionaryRecord match {
        case Nil => true
        case (ch, cnt) :: xs if occurenceMap(ch) >= cnt => goodFor(got)(xs)
        case _ => false
      }
    }
