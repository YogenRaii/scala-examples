...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def primary(col: String): Boolean = col.startsWith("t01") || col.startsWith("t03") || col.startsWith("t11") || col.startsWith("t1801") || col.startsWith("t1803")

    def working(col: String): Boolean = col.startsWith("t05") || col.startsWith("t1805")

    def other(col: String): Boolean =
      col.startsWith("t02") ||
      col.startsWith("t04") ||
      col.startsWith("t06") ||
      col.startsWith("t07") ||
    ...
    (columnNames.filter(primary).map(column), columnNames.filter(working).map(column), columnNames.filter(other).map(column))
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(num: Double, d: Int): Double = (num * pow(10, d)).round / 10d
