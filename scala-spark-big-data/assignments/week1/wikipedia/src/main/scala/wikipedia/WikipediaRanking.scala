...
object WikipediaRanking extends WikipediaRankingInterface {
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index.map(p => (p._1, p._2.size)).sortBy(-_._2).collect().toList
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd.flatMap({
    article => langs.filter(lang => article.mentionsLanguage(lang)).map(lang => (lang, 1))
